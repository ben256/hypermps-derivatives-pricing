...
def eval_tt_batched(tt_cores, x_indices):
	"""Evaluate TT cores for given discrete index per dimension, returning [batch] values.
	Does not modify global utils; used locally to ensure correct batching semantics.
	"""
	v = tt_cores[0][:, :, x_indices[0], :]
	for i in range(1, len(tt_cores)):
		v_next = tt_cores[i][:, :, x_indices[i], :]
		v = torch.bmm(v, v_next)
	# Sum over rank dims to get scalar per batch (works even if boundary ranks > 1)
	return v.sum(dim=(1, 2))


def build_ranks(format: str, d: int, N: int, max_rank: int):
	if format == 'TT':
		# Include boundary ranks = 1 for well-formed TT
		...
		n_model = 2
		num_cores = d * k
		# Symmetric rank growth up to max_rank then mirrored
		r = [1]
		for i in range(num_cores):
			if len(r) < (num_cores // 2) + 1:
				r.append(min(r[-1] * 2, max_rank))
			else:
				r.append(min(r[-1] * 2, max_rank))
				break
		r.extend(r[::-1][1:])
...
def measure_latency_and_memory(model, dataloader, format: str, d: int, N: int, device: torch.device, warmup: int = 2, iters: int = 10):
	...
	with torch.no_grad():
		for _ in range(iters):
			...
			tt_cores = model(params)
			if format == 'TT':
				output = torch.stack([eval_tt_batched(tt_cores, [i] * d) for i in range(N)], dim=1)
			else:
...
def run_single_format(
	...
	ranks, n_model = build_ranks(format, d, N, max_rank)
